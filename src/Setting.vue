<template>
    <van-pull-refresh v-model="loading" @refresh="onRefresh" :pulling-text="t('common.pulling-text')"
        ...
        <van-cell-group :title="t('setting.xrayhelper')" inset>
            ...
            <van-popover v-if="config.xrayHelper.coreType !== 'mihomo'" :actions="boolc" @select="(choose) => {config.xrayHelper.allowInsecure = choose.value;saveConfig();}" placement="bottom-end">
                <template #reference>
        ...
        <van-cell-group :title="t('setting.adghome')" inset>
            <van-popover :actions="boolc" @select="(choose) => {config.adgHome.enable = choose.value;saveConfig();}" placement="bottom-end">
                <template #reference>
        ...
        <van-cell-group :title="t('setting.proxy')" inset>
            <van-popover :actions="method" @select="(m) => {config.proxy.method = m.value;saveConfig();}" placement="bottom-end">
                <template #reference>
            ...
            <van-popover :actions="boolc" @select="(choose) => {config.proxy.enableIPv6 = choose.value;saveConfig();}" placement="bottom-end">
                <template #reference>
            ...
            <van-popover :actions="boolc" @select="(choose) => {config.proxy.autoDNSStrategy = choose.value;saveConfig();}" placement="bottom-end">
                <template #reference>
            ...
            <van-popover v-if="config.proxy.method !== 'tun'" :actions="mode" @select="(m) => {config.proxy.mode = m.value;saveConfig();}" placement="bottom-end">
                <template #reference>
...
const checkCoreBin = (corePath) => {
    execCmdWithExitCode(`ls ${corePath}`).then(errno => {
        if(errno!==0){
            showConfirmDialog({
                message: t('setting.core-not-found'),
            }).then(() => {
                // on close
                receiver.value = true
                setTimeout(() => {
        ...
        callApi("get status").then(value => {
            if (value.pid.length > 0) {
                execXrayHelperCmd("proxy refresh").then(() => {
                    showToast(t('setting.refresh-proxy'))
                })
            }
        })
    }, 50)
    ...
    if (runningWarn) {
        callApi("get status").then(status => {
            if (typeof status.pid == "string" && status.pid.length > 0) {
                showConfirmDialog({
                    title: t('common.tips'),
                    message: t('setting.running-warn'),
                    confirmButtonText: t('common.confirm-text'),
                    cancelButtonText: t('common.ignore-text')
                }).catch(() => {
